...
const app = express();
const SECRET_KEY = process.env.JWT_SECRET_KEY_CHAT_APP;

mongoose.connect(process.env.MONGO_URL_CHAT_APP)
    .then(() => console.log("MongoDB of chatApp connected"))
    .catch(err => console.error("MongoDB of chatApp connection error:", err));



const newsWeatherRoutes = require('./sideServer/NWAserver');
const fakeStoreRoutes = require('./sideServer/FSserver');


app.use('/newsweather', newsWeatherRoutes);
...
//處理獲取所有用戶資訊(包含在線、離線)
app.get("/chat/people", async (req, res) => {
